/*
  ______         _____ _______   ____          ________ ____  
 |___  /   /\   |  __ \_   _\ \ / /\ \        / /  ____|  _ \ 
    / /   /  \  | |__) || |  \ V /  \ \  /\  / /| |__  | |_) |
   / /   / /\ \ |  ___/ | |   > <    \ \/  \/ / |  __| |  _ < 
  / /__ / ____ \| |    _| |_ / . \    \  /\  /  | |____| |_) |
 /_____/_/    \_\_|   |_____/_/ \_\    \/  \/   |______|____/ 
				                      ZAPiX Web
W H A T S A P P W E B   数 据 提 取 器						

ZAPiXWEB WhatsApp 数据提取器 - 适用于 CHROME、FIREFOX、EDGE 和 OPERA
(也支持从浏览器中离线提取数据)

脚本名称：SPIZAPIXWEB_zh-CN.js

说明：用于提取 WhatsApp Web 数据记录的脚本。
技术原理见以下论文：
Soares, A. (2022). WhatsApp Web Client Live Forensics Technique. In Proceedings of the 8th International Conference on Information Systems Security and Privacy - ICISSP, ISBN 978-989-758-553-1, pages 629-636. DOI: 10.5220/0011006400003120

版权：2021 Alberto Magno <alberto.magno@gmail.com>

用法：将全部代码复制到浏览器控制台，然后按回车键执行。
按照控制台中的提示操作。
在 Firefox 中，需要先在控制台手动输入 "allow paste" 或 "allow pasting" 以允许粘贴。
-----------------------------------------------------------------

作者：alberto.magno@gmail.com (https://github.com/kraftdenker)  _
*/

//-------------------------------
// ZAPIX 引入
//-------------------------------
window.ZAPiX = {};
window._ZAPiX = {};

window.ZAPiX._serializeRawObj = (obj) => {
    if (obj && obj.toJSON) {
        return obj.toJSON();
    }
    return {}
};

window.ZAPiX._getContactPhoneMetadata = function (obj) {
    if (!obj) return {};
    var rawId = obj.id && obj.id._serialized ? String(obj.id._serialized) : String(obj.id || '');
    var phoneValue = obj.phoneId || obj.pn || obj.phoneNumber || obj.phone || null;
    if (!phoneValue && /@c\.us$/i.test(rawId)) phoneValue = rawId;
    if (!phoneValue && /@lid$/i.test(rawId)) {
        try {
            var apiContact = require("WAWebApiContact");
            if (apiContact && typeof apiContact.getPhoneNumber === 'function') {
                phoneValue = apiContact.getPhoneNumber(obj.id);
            }
        } catch (error) {
            console.warn('ZAPiX: Unable to resolve phone number for '+rawId, error);
        }
    }
    if (phoneValue && typeof phoneValue.then === 'function') return {};
    var phoneId = '';
    if (typeof phoneValue === 'string' || typeof phoneValue === 'number') {
        phoneId = String(phoneValue);
    } else if (phoneValue && phoneValue._serialized) {
        phoneId = String(phoneValue._serialized);
    } else if (phoneValue && phoneValue.id && phoneValue.id._serialized) {
        phoneId = String(phoneValue.id._serialized);
    } else if (phoneValue && phoneValue.user) {
        phoneId = String(phoneValue.user)+'@c.us';
    }
    if (/^\+?\d{7,15}$/.test(phoneId)) phoneId = phoneId.replace(/^\+/, '')+'@c.us';
    var phoneMatch = phoneId.match(/^(\+?\d{7,15})(?:(?::|_)(\d+))?@c\.us$/i);
    if (!phoneMatch) return {};
    return {
        phoneId: phoneId,
        phoneNumber: phoneMatch[1].replace(/^\+/, ''),
        deviceId: phoneMatch[2] || ''
    };
};

window.ZAPiX._lastDigest = undefined;
window.ZAPiX._statusTextnode = undefined;
window.ZAPiX._zip = undefined;
window.ZAPiX._injectedStatusNode = undefined;
window.ZAPiX._injectedHeaderNode = undefined;
// --- 对照表
window.ZAPiX._decodeZapMedia = {
    "image": "WhatsApp Image Keys",
    "sticker": "WhatsApp Image Keys",
    "video": "WhatsApp Video Keys",
    "audio": "WhatsApp Audio Keys",
    "document": "WhatsApp Document Keys",
    "image/webp": "WhatsApp Image Keys",
    "image/jpeg": "WhatsApp Image Keys",
    "image/png": "WhatsApp Image Keys",
    "video/mp4": "WhatsApp Video Keys",
    "audio/aac": "WhatsApp Audio Keys",
    "audio/ogg": "WhatsApp Audio Keys",
    "audio/wav": "WhatsApp Audio Keys",
	"ptt": "WhatsApp Audio Keys"
}

window.ZAPiX._mediaTypes = new Set();

window.ZAPiX._fileExtensions = JSON.parse('{"audio/aac": ".aac","application/x-abiword": ".abw","application/octet-stream": ".abw","video/x-msvideo": ".avi","application/vnd.amazon.ebook": ".azw","application/octet-stream": ".bin","image/bmp": ".bmp","application/x-bzip": ".bz","application/x-bzip2":".bz2","application/x-csh": ".csh","text/css": ".css","text/csv": ".csv","application/msword": ".doc","application/vnd.openxmlformats-officedocument.wordprocessingml.document": ".docx","application/vnd.ms-fontobject": ".eot","application/epub+zip": ".epub","video/x-flv": ".flv","image/gif": ".gif","text/html": ".htm","text/html": ".html","image/x-icon": ".ico","text/calendar": ".ics","application/java-archive": ".jar","image/jpeg": ".jpeg","text/javascript": ".js","application/json": ".json","audio/midi": ".midi","audio/x-midi": ".midi","video/x-matroska": ".mkv","audio/mpeg": ".mp3","audio/mp3": ".mp3","video/mpeg": ".mpeg","application/vnd.apple.installer+xml": ".mpkg","application/vnd.oasis.opendocument.presentation": ".odp","application/vnd.oasis.opendocument.spreadsheet": ".ods","application/vnd.oasis.opendocument.text": ".odt","audio/ogg": ".oga","audio/ogg; codecs=opus": ".opus","video/ogg": ".ogv","application/ogg": ".ogx","font/otf": ".otf","image/png": ".png","application/pdf": ".pdf","application/vnd.ms-powerpoint": ".ppt","application/vnd.openxmlformats-officedocument.presentationml.presentation": ".pptx","application/x-rar-compressed": ".rar","application/rtf": ".rtf","application/x-sh": ".sh","image/svg+xml": ".svg","application/x-shockwave-flash": ".swf","application/x-tar": ".tar","image/tiff": ".tiff","application/typescript": ".ts","font/ttf": ".ttf","text/plain": ".txt","text/x-vcard": ".vcf","application/vnd.visio": ".vsd","audio/wav": ".wav","audio/webm": ".weba","video/webm": ".webm","image/webp": ".webp","font/woff": ".woff","font/woff2": ".woff2","application/xhtml+xml": ".xhtml","application/vnd.ms-excel": ".xls","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": ".xlsx","application/xml": ".xml","text/xml": ".xml","application/vnd.mozilla.xul+xml": ".xul","application/zip": ".zip","application/x-zip-compressed": ".zip","video/3gpp": ".3gp","audio/3gpp": ".3gp","video/3gpp2": ".3g2","audio/3gpp2": ".3g2","application/x-7z-compressed": ".7z","application/vnd.google-earth.kmz": ".kmz","video/mp4":".mp4","undef": ".undf", "undefined":".undf"}');

window.ZAPiX._zapix_header = '     _|_|_|   _|_|_|     |_|_|	\n    _|        _|  _|       _|	\n      _|_|    _|_|_|       _|	\n         _|   _|           _|	\n    _|_|_|    _|         _|_|_|	\n	                      ZAPiX Web\nW H A T S A P P W E B  数 据 提 取 器';

window.ZAPiX._logo = '';

// META 模块
const fflate = require("fflate");
const msgHelper = require("WAWebMsgGetters");
const contactHelper = require("WAWebContactGetters");

const contactCollection = (require("WAWebContactCollection")).ContactCollection;
const chatCollection = (require("WAWebChatCollection")).ChatCollection;
const profilePicThumbCollection = (require("WAWebProfilePicThumbCollection")).ProfilePicThumbCollection;
const userAccountData = require("WAWebUserPrefsMeUser");

window.ZAPiX._zipFiles = {};

function addFile2Zip(fileName, payload) {
    window.ZAPiX._zipFiles[fileName] = payload;
}
// ------ 工具函数

var blobfy = function (obj){
	const str = JSON.stringify(obj);
	const bytes = new TextEncoder().encode(str);
	//const blob = new Blob([bytes], {    type: "application/json;charset=utf-8"});
	return bytes;

}

function htmlToPlainText(html) {
    const tempElement = document.createElement('div');
    tempElement.innerHTML = html;

    // 从 <img> 标签中提取文本内容和 alt 属性
    const imgTags = tempElement.querySelectorAll('img');
    imgTags.forEach((img) => {
        const altText = img.getAttribute('alt');
        if (altText) {
            img.insertAdjacentText('afterend', `${altText}`);
        }
    });

    return tempElement.textContent;
}

function base64ToArrayBuffer(base64) {
    var binary_string = window.atob(base64);
    var len = binary_string.length;
    var bytes = new Uint8Array(len);
    for (var i = 0; i < len; i++) {
        bytes[i] = binary_string.charCodeAt(i);
    }
    return bytes.buffer;
}

function isElectron() {
    // 渲染进程
    if (typeof window !== 'undefined' && typeof window.process === 'object' && window.process.type === 'renderer') {
        return true;
    }

    // 主进程
    if (typeof process !== 'undefined' && typeof process.versions === 'object' && !!process.versions.electron) {
        return true;
    }

    // 当 `nodeIntegration` 选项设为 true 时检测用户代理
    if (typeof navigator === 'object' && typeof navigator.userAgent === 'string' && navigator.userAgent.indexOf('Electron') >= 0) {
        return true;
    }

    return false;
}


var b64toBlob = (b64Data, contentType='', sliceSize=512) => {
  const byteCharacters = atob(b64Data);
  const byteArrays = [];

  for (let offset = 0; offset < byteCharacters.length; offset += sliceSize) {
    const slice = byteCharacters.slice(offset, offset + sliceSize);

    const byteNumbers = new Array(slice.length);
    for (let i = 0; i < slice.length; i++) {
      byteNumbers[i] = slice.charCodeAt(i);
    }

    const byteArray = new Uint8Array(byteNumbers);
    byteArrays.push(byteArray);
  }

  const blob = new Blob(byteArrays, {type: contentType});
  return blob;
}

window.ZAPiX._serializeRawObj = (obj) => {
    if (obj && obj.toJSON) {
        return obj.toJSON();
    }
    return {}
};

/**
 * 序列化聊天对象
 *
 * @param rawChat 聊天对象
 * @returns {{}}
 */

window.ZAPiX._serializeChatObj = (obj) => {
    if (obj == undefined) {
        return null;
    }
    return Object.assign(window.ZAPiX._serializeRawObj(obj), window.ZAPiX._getContactPhoneMetadata(obj), {
        id: obj.id._serialized,
        kind: obj.kind,
        isGroup: obj.isGroup,
        formattedTitle: obj.formattedTitle,
        contact: obj['contact'] ? window.ZAPiX._serializeContactObj(obj['contact']) : null,
        groupMetadata: obj["groupMetadata"] ? window.ZAPiX._serializeRawObj(obj["groupMetadata"]) : null,
        presence: obj["presence"] ? window.ZAPiX._serializeRawObj(obj["presence"]) : null,
        msgs: null
    });
};

window.ZAPiX._serializeContactObj = (obj) => {
    if (obj == undefined) {
        return null;
    }
    return Object.assign(window.ZAPiX._serializeRawObj(obj), window.ZAPiX._getContactPhoneMetadata(obj), {
        id: obj.id._serialized,
        formattedName: obj.formattedName,
        isHighLevelVerified: obj.isHighLevelVerified,
        isMe: obj.isMe,
        isMyContact: obj.isMyContact,
        isPSA: obj.isPSA,
        isUser: obj.isUser,
        isVerified: obj.isVerified,
        isWAContact: obj.isWAContact,
        profilePicThumbObj: obj.profilePicThumb ? ZAPiX._serializeProfilePicThumb(obj.profilePicThumb) : {},
        statusMute: obj.statusMute,
        msgs: null
    });
};


window.ZAPiX._serializeMessageObj = (obj) => {
    if (obj == undefined) {
        return null;
    }
    const _chat = obj['chat'] ? ZAPiX._serializeChatObj(obj['chat']) : {};
    if (obj.quotedMsg) obj.quotedMsgObj;
    return Object.assign(window.ZAPiX._serializeRawObj(obj), {
        id: obj.id._serialized,
        from: obj.from._serialized,
        quotedParticipant: obj.quotedParticipant ? obj.quotedParticipant._serialized ? obj.quotedParticipant._serialized : undefined : undefined,
        author: obj.author ? obj.author._serialized ? obj.author._serialized : undefined : undefined,
        chatId: obj.chatId ? obj.chatId._serialized ? obj.chatId._serialized : undefined : undefined,
        to: obj.to ? obj.to._serialized ? obj.to._serialized : undefined : undefined,
        fromMe: obj.id.fromMe,
        sender: obj["senderObj"] ? ZAPiX._serializeContactObj(obj["senderObj"]) : null,
        timestamp: obj["t"],
        content: obj["body"],
        isGroupMsg: obj.isGroupMsg,
        isLink: obj.isLink,
        isMMS: obj.isMMS,
        isMedia: obj.isMedia,
        isNotification: obj.isNotification,
        isPSA: obj.isPSA,
        type: obj.type,
        chat: _chat,
        isOnline: _chat.isOnline,
        lastSeen: _chat.lastSeen,
        chatId: obj.id.remote,
        quotedMsgObj: window.ZAPiX._serializeMessageObj(obj['_quotedMsgObj']),
        mediaData: window.ZAPiX._serializeRawObj(obj['mediaData']),
        reply: body => window.ZAPiX.reply(_chat.id._serialized, body, obj)
    });
};

window.ZAPiX._serializeNumberStatusObj = (obj) => {
    if (obj == undefined) {
        return null;
    }

    return Object.assign({}, {
        id: obj.jid,
        status: obj.status,
        isBusiness: (obj.biz === true),
        canReceiveMessage: (obj.status === 200)
    });
};

window.ZAPiX._serializeProfilePicThumb = (obj) => {
    if (obj == undefined) {
        return null;
    }

    return Object.assign({}, {
        eurl: obj.eurl,
        id: obj.id,
        img: obj.img,
        imgFull: obj.imgFull,
        raw: obj.raw,
        tag: obj.tag
    });
}


// 主要任务函数
window.ZAPiX._getDevicePhoneMetadata = function (){
	var devicePn = null;
	try{
		devicePn = userAccountData.getMaybeMeDevicePn();
	}catch(error){
		console.warn('ZAPiX：无法读取当前设备手机号。', error);
	}
	var phoneId = devicePn && devicePn._serialized ? String(devicePn._serialized) : '';
	var phoneNumber = devicePn && devicePn.user != undefined ? String(devicePn.user) : '';
	var deviceId = devicePn && devicePn.device != undefined ? String(devicePn.device) : '';
	var phoneMatch = phoneId.match(/^(\+?\d+)(?:(?::|_)(\d+))?@c\.us$/i);
	if (!phoneNumber && phoneMatch) phoneNumber = phoneMatch[1].replace(/^\+/, '');
	if (!deviceId && phoneMatch) deviceId = phoneMatch[2] || '';
	return {
		phoneId: phoneId,
		phoneNumber: phoneNumber.replace(/\D/g, ''),
		deviceId: deviceId
	};
}

window.ZAPiX._getUser = function (){
	var metadata = window.ZAPiX._getDevicePhoneMetadata();
	return (metadata.phoneId || 'unknown@c.us').replaceAll('\"','').replaceAll(":","_");
	//return (localStorage.getItem("last-wid-md")).replaceAll('\"','').replaceAll(":","_");
}

window.ZAPiX._serializeCurrentUser = function (userAccount){
	var serialized = window.ZAPiX._serializeContactObj(userAccount) || {};
	return Object.assign({}, serialized, window.ZAPiX._getDevicePhoneMetadata());
}

window.ZAPiX._downloadFile2Array = async function (url) {
    if (!url) return false;
    //console.log(url);
    const response = await fetch(url);
    const ab = await response.arrayBuffer();
    return btoa(new Uint8Array(ab).reduce((data, byte) => data + String.fromCharCode(byte), ''));
};


window.ZAPiX._getProfilesPics = async function(contacts){
    for(var i = 0; i < contacts.length; i++) {
		var obj = contacts[i];
		window.ZAPiX._statusTextnode.data = "头像 "+obj.id+" - "+i+"/"+contacts.length;
		console.log("正在读取数据，ID："+obj.id);
        picArray = profilePicThumbCollection._models.filter((p)=>p.__x_id._serialized === obj.id);
        if (picArray.length > 0){
            //console.log('URL:');
            url = picArray[0].__x_imgFull;
            //console.log(url);
            if (typeof url !== 'undefined' && url !== null) {
                try{
                    const response = await fetch(url);
                    const data = await response.arrayBuffer();
                    addFile2Zip("Avatar "+obj.id+".jpg", [new Uint8Array(data),{ level:0}]);
                    console.log("头像 "+obj.id+" 已保存");
                } catch (error) {
                    console.error("头像 "+obj.id+" 保存失败 - ", error);
                }
            }
        }
		
	}
}

window.ZAPiX._downloadMediaFile = async function(msg) {
    
	console.log("正在下载媒体文件");
	//console.log(msg);
    
    var url = msgHelper.getDeprecatedMms3Url(msg);
	var dec_media_withpad = base64ToArrayBuffer(await window.ZAPiX._downloadFile2Array(url));
	//console.log(dec_media_withpad);
	var dec_media = dec_media_withpad.slice(0,dec_media_withpad.byteLength-10);
	//console.log(dec_media);
    var mediaType = msgHelper.getType(msg);
	window.ZAPiX._mediaTypes.add(mediaType);
	var mediakey = msgHelper.getMediaKey(msg);
	console.log(window.ZAPiX._decodeZapMedia[mediaType]);
    var mediaKeyExpanded = 'notExpanded'
    mediaHkdf = require("WACryptoHkdf");
	//console.log(mediakey);
    var mediaKeyExpanded = await mediaHkdf.extractAndExpand(base64ToArrayBuffer(mediakey), window.ZAPiX._decodeZapMedia[mediaType], 112);
    //mediaKeyExpanded.then((data)=> expandedKey = data);
    //console.log(mediaKeyExpanded);
	var ivd = new Uint8Array(mediaKeyExpanded.slice(0, 16));
	//console.log('IV:'+ivd+" 字节长度:"+ivd.byteLength);
    var cipherKey = new Uint8Array(mediaKeyExpanded.slice(16,48));
	//console.log('密钥:'+cipherKey+" 字节长度:"+cipherKey.byteLength);
	var decAESkey = await crypto.subtle.importKey('raw',cipherKey,{ name: 'AES-CBC' },false, ['decrypt']);
	//console.log('decAESKey:');
    //console.log(decAESkey);
    var decoded = await crypto.subtle.decrypt({name:'AES-CBC', iv:new Uint8Array(ivd)}, decAESkey, dec_media);
	//console.log('decrypted:');
	//console.log(decoded);
	if (decoded.byteLength == 0)
	{
		throw new Error('媒体文件解密后内容为空');
	} else {
        console.log('正在保存：','Attachment '+msgHelper.getId(msg)+window.ZAPiX._fileExtensions[msgHelper.getMimetype(msg)]);
		addFile2Zip('Attachment '+msgHelper.getId(msg)+window.ZAPiX._fileExtensions[msgHelper.getMimetype(msg)], [new Uint8Array(decoded),{ level:0}]);
		console.log('媒体文件解密成功');
		return true;
	}
};

window.ZAPiX._downloadMediaFileWithRetry = async function(msg, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            var url = msgHelper.getDeprecatedMms3Url(msg);
            if (!url) {
                console.log('ZAPiX：媒体文件 ' + msgHelper.getId(msg) +
                           ' 没有可用 URL，尝试 ' + attempt + '/' + maxRetries);
                if (msg.mediaData && typeof msg.mediaData.downloadMedia === 'function') {
                    console.log('ZAPiX：正在尝试 mediaData.downloadMedia()...');
                    await msg.mediaData.downloadMedia();
                    await new Promise(r => setTimeout(r, 5000));
                    url = msgHelper.getDeprecatedMms3Url(msg);
                }
                if (!url) {
                    if (attempt < maxRetries) {
                        await new Promise(r => setTimeout(r, 3000 * attempt));
                        continue;
                    }
                    throw new Error('尝试 ' + maxRetries + ' 次后仍没有可用的媒体 URL');
                }
            }
            var msgType = msgHelper.getType(msg);
            if (msgType === 'sticker' && msg.mediaData && typeof msg.mediaData.downloadMedia === 'function') {
                console.log('ZAPiX：正在使用贴纸专用下载方式...');
                await msg.mediaData.downloadMedia();
                await new Promise(r => setTimeout(r, 3000));
                url = msgHelper.getDeprecatedMms3Url(msg);
                if (!url && attempt < maxRetries) {
                    await new Promise(r => setTimeout(r, 3000 * attempt));
                    continue;
                }
            }
            await window.ZAPiX._downloadMediaFile(msg);
            return {
                downloaded: true,
                previewSaved: false,
                error: null
            };
        } catch(e) {
            console.log('ZAPiX：第 ' + attempt + ' 次下载失败：' + e.message);
            if (attempt === maxRetries) {
                console.log('ZAPiX：所有重试均已失败，正在尝试保存预览数据...');
                var previewSaved = false;
                try {
                    previewSaved = await window.ZAPiX._internal_saveAttachmentPreview(msg);
                } catch(pe) {
                    console.log('ZAPiX：预览数据也保存失败：' + pe.message);
                }
                addFile2Zip('FAILED_' + msgHelper.getId(msg) + '.json',
                    [blobfy({
                        msgId: msgHelper.getId(msg),
                        type: msgHelper.getType(msg),
                        error: e.message,
                        hasMediaKey: !!msgHelper.getMediaKey(msg),
                        timestamp: msg.__x_t || msg.t
                    }), { level: 9 }]);
                return {
                    downloaded: false,
                    previewSaved: previewSaved === true,
                    error: e.message
                };
            }
        }
    }
    return {
        downloaded: false,
        previewSaved: false,
        error: '媒体下载结束，但没有返回结果'
    };
};

window.ZAPiX._filterPhantomMessages = function(rawMsgs) {
    return rawMsgs.filter(function(msg) {
        var mType = msgHelper.getType(msg);
        var body = msgHelper.getBody(msg);
        if ((mType === 'e2e_notification' || mType === 'protocol') &&
            !body && !msgHelper.getMediaKey(msg)) {
            return false;
        }
        if (body && (body.indexOf('No se pudo cargar este mensaje') !== -1 ||
                     body.indexOf('Couldn\'t load this message') !== -1 ||
                     body.indexOf('无法加载此消息') !== -1 ||
                     body.indexOf('无法加载该消息') !== -1)) {
            return false;
        }
        return true;
    });
};

window.ZAPiX._processAlbumMetadata = function(msgs, chatId) {
    var albums = {};
    for (var i = 0; i < msgs.length; i++) {
        var msg = msgs[i];
        if (msg.__x_isAlbum && msg.__x_albumMsgs) {
            var albumId = msgHelper.getId(msg);
            albums[albumId] = {
                albumHeaderId: albumId,
                timestamp: msg.__x_t || msg.t,
                childMsgIds: msg.__x_albumMsgs.map(function(m) { return m._serialized || m; }),
                childCount: msg.__x_albumMsgs.length,
                type: 'album_header'
            };
        }
        if (msg.__x_parentMsgKey) {
            var parentKey = msg.__x_parentMsgKey._serialized || msg.__x_parentMsgKey;
            if (!albums[parentKey]) {
                albums[parentKey] = {
                    albumHeaderId: parentKey,
                    childMsgIds: [],
                    type: 'album_reconstructed'
                };
            }
            albums[parentKey].childMsgIds.push(msgHelper.getId(msg));
        }
    }
    if (Object.keys(albums).length > 0) {
        console.log('ZAPiX：在聊天 ' + chatId + ' 中找到 ' + Object.keys(albums).length + ' 个相册');
        addFile2Zip('album_metadata_' + chatId + '.json', [blobfy(albums), { level: 9 }]);
    }
    return albums;
};

window.ZAPiX._triggerSync = async function(maxWaitMs) {
    if (typeof maxWaitMs === 'undefined') maxWaitMs = 120000;
    var banners = document.querySelectorAll('[role="button"]');
    var syncBanner = null;
    for (var i = 0; i < banners.length; i++) {
        var b = banners[i];
        if (b.textContent.indexOf('obtener mensajes anteriores') !== -1 ||
            b.textContent.indexOf('Get earlier messages') !== -1 ||
            b.textContent.indexOf('获取较早的消息') !== -1 ||
            b.textContent.indexOf('获取更早的消息') !== -1) {
            syncBanner = b;
            break;
        }
    }
    if (!syncBanner) {
        console.log('ZAPiX：未找到同步提示，所有消息可能已加载完成');
        return true;
    }
    syncBanner.click();
    console.log('ZAPiX：已触发同步，正在等待消息...');
    var startTime = Date.now();
    while (Date.now() - startTime < maxWaitMs) {
        await new Promise(function(r) { setTimeout(r, 3000); });
        var stillSyncing = document.querySelector('[data-icon="spinner-light"]') ||
                          document.querySelector('[data-icon="spinner"]');
        if (!stillSyncing) {
            console.log('ZAPiX：同步似乎已完成，耗时 ' +
                        ((Date.now() - startTime) / 1000) + ' 秒');
            return true;
        }
    }
    console.log('ZAPiX：同步已超时');
    return false;
};

window.ZAPiX._getUserContacts =  function () {
    return contactCollection._models.filter((contact) => contact.syncToAddressbook==true && (contactHelper.getCanRequestPhoneNumber(contact) === true || contactHelper.getIsMe(contact) === true)).map((contact) => window.ZAPiX._serializeContactObj(contact));
};

window.ZAPiX._getAllChats = function () {
    return chatCollection._models.map((chat) => window.ZAPiX._serializeChatObj(chat));
};

window.ZAPiX._internal_start = async function(){
	window.ZAPiX._statusTextnode.data = "正在提取用户账户...";
	var userAccount = contactCollection.getMeContact(); //await userAccountData.getMe();
	var userAccountPayload = window.ZAPiX._serializeCurrentUser(userAccount);
	window.ZAPiX._statusTextnode.data = "正在提取用户联系人...";
	var contacts = await window.ZAPiX._getUserContacts();
	window.ZAPiX._statusTextnode.data = "正在保存用户账户...";
    addFile2Zip("userAccount.json", [blobfy(userAccountPayload),{ level:9}]); 
	console.log('用户账户 - 完成');
	window.ZAPiX._statusTextnode.data = "正在保存用户联系人...";
    addFile2Zip("contacts.json", [blobfy(contacts), {level:9}]);

    console.log('用户联系人 - 完成');
	window.ZAPiX._statusTextnode.data = "正在提取用户近期聊天...";
	var chats = await window.ZAPiX._getAllChats();
	window.ZAPiX._statusTextnode.data = "正在保存用户聊天...";
    addFile2Zip("chats.json", [blobfy(chats),{level:9}]);
    
    console.log('用户聊天 - 完成');
	window.ZAPiX._statusTextnode.data = "正在提取头像...";
	console.log('正在下载用户头像');
	try{
		await window.ZAPiX._getProfilesPics(contacts);
	}
	catch(e){
		console.log('提取用户头像时出错');
	}
	window.ZAPiX._statusTextnode.data = "账户、联系人和头像数据已采集";
}

window.ZAPiX._internal_saveAttachmentPreview = async function(msg){
	try{
		var blob;
        console.log(msg);
        if (msgHelper.getBody(msg)!=''){
            console.log(msgHelper.getBody(msg))
            console.log('正在调用 b64toBlob')
            blob = b64toBlob(msgHelper.getBody(msg));
            console.log('b64toBlob 调用完成')
        }
		var mimetypeFileExtension = window.ZAPiX._fileExtensions[msgHelper.getMimetype(msg)];
		if (msg['type']=='vcard'){
			mimetypeFileExtension = window.ZAPiX._fileExtensions['text/x-vcard'];
		}else{
			//blob = msgHelper.getMediadatamsg['mediaData']['fullPreviewData']['_blob'];
			mimetypeFileExtension = window.ZAPiX._fileExtensions[msg['mimetype']]+window.ZAPiX._fileExtensions[msg['mediaData']['fullPreviewData']['_mimetype']];
		}
		if (blob == undefined || blob.size === 0){
			throw new Error('没有可用的内嵌预览数据');
		}
		//window.ZAPiX._zip.addFile(blob, {filename: stringToByteArray('Attachment '+msg['id']+'.embedded'+mimetypeFileExtension)});
		console.log('Attachment '+msg['id']+'.embedded'+mimetypeFileExtension);
        var previewBytes = new Uint8Array(await blob.arrayBuffer());
        addFile2Zip('Attachment '+msg['id']+'.embedded'+mimetypeFileExtension, [previewBytes,{ level:0}]);
		console.log("已保存");
		return true;
	}catch(e){
		console.log("保存失败 - "+e.message);
		return false;
	}
}

window.ZAPiX._internal_getallchats = async function (){
    var chats = chatCollection._models;
	var chatCount = chats.length;
	var attachmentTypes = {
		image: true,
		video: true,
		ptt: true,
		audio: true,
		document: true,
		sticker: true,
		vcard: true
	};
	var summary = {
		chatCount: chatCount,
		totalAttachments: 0,
		processedAttachments: 0,
		downloadedMedia: 0,
		previewSaved: 0,
		vcardSaved: 0,
		failed: 0
	};
	for (var countChatIndex = 0; countChatIndex < chatCount; countChatIndex++) {
		var countMsgs = window.ZAPiX._filterPhantomMessages(chats[countChatIndex].msgs._models);
		for (var countMsgIndex = 0; countMsgIndex < countMsgs.length; countMsgIndex++) {
			if (attachmentTypes[msgHelper.getType(countMsgs[countMsgIndex])] === true) {
				summary.totalAttachments++;
			}
		}
	}
	window.ZAPiX._statusTextnode.data = "正在提取用户聊天消息和附件...";
	console.log('正在从 '+chatCount+' 个聊天中获取消息');
	console.log('ZAPiX：共发现 '+summary.totalAttachments+' 个附件，下面将显示处理进度。');
	for (var i = 0; i < chatCount; i++) {
		chatName = chats[i].__x_formattedTitle;
		var rawMsgs = chats[i].msgs._models;
		var msgs = window.ZAPiX._filterPhantomMessages(rawMsgs);
		if (msgs.length < rawMsgs.length) {
			console.log('ZAPiX：已从聊天 ' + chats[i].__x_id + ' 中过滤 ' + (rawMsgs.length - msgs.length) + ' 条幽灵消息');
		}
		addFile2Zip('Chat '+chats[i].__x_id+'.json', [blobfy(msgs),{ level:9}]);
		window.ZAPiX._processAlbumMetadata(msgs, chats[i].__x_id);
        chatName = chats[i].__x_formattedTitle;
		for (var m=0; m < msgs.length; m++){
			msgType = msgHelper.getType(msgs[m]);
			if (msgType=='image' || msgType=='video' || msgType=='ptt' || msgType=='audio' || msgType=='document' || msgType=='sticker'){
				summary.processedAttachments++;
				var mediaProgress = '['+summary.processedAttachments+'/'+summary.totalAttachments+']';
				window.ZAPiX._statusTextnode.data = "聊天 "+chatName+" - "+(i+1)+"/"+chatCount+" | 附件 "+summary.processedAttachments+"/"+summary.totalAttachments;
				console.log('ZAPiX：'+mediaProgress+' 正在下载 '+msgType+' '+msgHelper.getId(msgs[m])+'...');
				try{
					var mediaResult = await window.ZAPiX._downloadMediaFileWithRetry(msgs[m]);
					if (mediaResult.downloaded) {
						summary.downloadedMedia++;
						console.log('ZAPiX：'+mediaProgress+' 原始附件下载并保存完成。');
					} else if (mediaResult.previewSaved) {
						summary.previewSaved++;
						console.warn('ZAPiX：'+mediaProgress+' 原文件下载失败，已改存内嵌预览。');
					} else {
						summary.failed++;
						console.error('ZAPiX：'+mediaProgress+' 附件未能保存。');
					}
				}catch(e){
					summary.failed++;
					console.error('ZAPiX：'+mediaProgress+' 下载 '+msgType+' 时出错：'+e.message);
				}
			}
			else if (msgType=='vcard'){
				summary.processedAttachments++;
				var vcardProgress = '['+summary.processedAttachments+'/'+summary.totalAttachments+']';
				window.ZAPiX._statusTextnode.data = "聊天 "+chatName+" - "+(i+1)+"/"+chatCount+" | 附件 "+summary.processedAttachments+"/"+summary.totalAttachments;
				console.log('ZAPiX：'+vcardProgress+' 正在保存 vCard '+msgHelper.getId(msgs[m])+'...');
				try{
					mimetypeFileExtension = window.ZAPiX._fileExtensions['text/x-vcard'];
					addFile2Zip('Attachment '+msgHelper.getId(msgs[m])+'.embedded'+mimetypeFileExtension, [fflate.strToU8(msgHelper.getBody(msgs[m])),{ level:9}]);
					summary.vcardSaved++;
					console.log('ZAPiX：'+vcardProgress+' vCard 已保存。');
				}catch(e){
					summary.failed++;
					console.error('ZAPiX：'+vcardProgress+' vCard 保存失败：'+e.message);
				}
			}
		}
		window.ZAPiX._statusTextnode.data = "聊天 "+chatName+" 已提取";
		console.log('ZAPiX：聊天 '+(i+1)+'/'+chatCount+' 已提取：'+chatName);
	}
	return summary;
}
	
window.ZAPiX._decodeHtmlEntities = function(str) {
    return str.replace(/&amp;/g, '&')
              .replace(/&lt;/g, '<')
              .replace(/&gt;/g, '>')
              .replace(/&quot;/g, '"')
              .replace(/&#39;/g, "'");
}

window.ZAPiX._internal_getChatByName = function(chatName){
	ret = null
	chatName = window.ZAPiX._decodeHtmlEntities(chatName).trimStart().trimEnd();
	chatName = window.htmlToPlainText(chatName)
	var chats = window.ZAPiX._getAllChats();
	for (var i = 0; i < chats.length; i++) {
		//console.log(chats[i].formattedTitle);
		if ((chats[i].isGroup && chats[i].name == chatName)||(!chats[i].isGroup && chats[i].formattedTitle == chatName)||(!chats[i].isGroup && chats[i].contact.name == chatName)){
			console.log("已找到："+chats[i].name)
			ret =  chats[i]
			break;
		}
	}
	return ret;
}

window.ZAPiX._internal_getchat = async function (chatName){
	window.ZAPiX._statusTextnode.data = "正在提取聊天 "+chatName+" 的消息和附件...";
	console.log('正在从聊天 '+chatName+' 中获取消息');
	chat = window.ZAPiX._internal_getChatByName(chatName);
	if (chat == null){
		console.log('未找到聊天：'+chatName+'。');
		window.ZAPiX._statusTextnode.data = '未找到聊天：'+chatName+'。';
		return;
	}
	console.log('正在从聊天 '+chat.id+' 中获取消息');
    resultOfFiltering = chatCollection._models.filter((c)=>c.__x_id._serialized === chat.id)
    if (resultOfFiltering.length > 0){
        var rawMsgs = resultOfFiltering[0].msgs._models;
        var msgs = window.ZAPiX._filterPhantomMessages(rawMsgs);
        if (msgs.length < rawMsgs.length) {
            console.log('ZAPiX：已从聊天 ' + chat.id + ' 中过滤 ' + (rawMsgs.length - msgs.length) + ' 条幽灵消息');
        }
        addFile2Zip('Chat '+chat.id+'.json', [blobfy(msgs),{ level:9}]);
        window.ZAPiX._processAlbumMetadata(msgs, chat.id);
		for (var m=0; m < msgs.length; m++){
            msgType = msgHelper.getType(msgs[m]);
            console.log(msgType);
			if (msgType=='image' || msgType=='video' || msgType=='ptt' || msgType=='audio' || msgType=='document' || msgType=='sticker'){
				try{
					await window.ZAPiX._downloadMediaFileWithRetry(msgs[m]);
				}catch(e){
					console.log('下载 '+msgType+' 时出错：'+e.message+"。将使用内嵌数据。");
				}
			} 
			else if (msgType=='vcard'){
				try{
					mimetypeFileExtension = window.ZAPiX._fileExtensions['text/x-vcard'];
					addFile2Zip('Attachment '+msgHelper.getId(msgs[m])+'.embedded'+mimetypeFileExtension, [fflate.strToU8(msgHelper.getBody(msgs[m])),{ level:9}]);
					console.log("vCard 已保存");
				}catch(e){
					console.log("vCard 保存失败 - "+e.message);
				}
			}
		}
    	window.ZAPiX._statusTextnode.data = chatName+" 已提取！";
    }
}
async function saveFile(dataFile, fileName) {
	if (!(dataFile instanceof Blob)){
		dataFile = new Blob([dataFile], {type:'text/plain'});
	}
	const opts = {
	suggestedName: fileName
	};
	const blobURL = URL.createObjectURL(dataFile);
    const link = document.createElement("a");
    link.href = blobURL;
    link.download = fileName;
    link.click();
    URL.revokeObjectURL(blobURL);
	
	/*const newHandle = await window.showSaveFilePicker(opts);
	const options = {};
	options.mode = 'readwrite';
	await newHandle.requestPermission(options);
	const writableStream = await newHandle.createWritable();
	await writableStream.write(dataFile);
	await writableStream.close();*/
}


window.ZAPiX.showDigest = async function(){
	var hashHex = window.ZAPiX._lastDigest;
	if (hashHex==undefined){
		var message = "尚未生成哈希值。请先完成“打包导出”并等待哈希计算结束。";
		if (window.ZAPiX._statusTextnode!=undefined)
			window.ZAPiX._statusTextnode.data = message;
		console.warn(message);
		alert(message);
		return false;
	}

	var userAccount = window.ZAPiX._getUser();
	var fileName = "ZAPiXWEB_"+userAccount+".sha512.txt";
	var content = hashHex +" ?SHA512*"+ "ZAPiXWEB_"+userAccount+".zip";
	await saveFile(content, fileName);
	if (window.ZAPiX._statusTextnode!=undefined)
		window.ZAPiX._statusTextnode.data = "哈希文件已下载："+fileName;
	console.log("哈希文件已下载："+fileName);
	return true;
}

window.ZAPiX._internal_end = async function (){
	var userAccount = window.ZAPiX._getUser();
    var fileName = "ZAPiXWEB_"+userAccount+".zip";
    var zip = fflate.zipSync(window.ZAPiX._zipFiles);
    var blobzip = new Blob([zip], { type: 'application/zip' });
    await saveFile(blobzip,fileName);
	window.ZAPiX._statusTextnode.data = "ZIP 文件已保存，正在计算 SHA-512 哈希值...";
	const hashBuffer = await crypto.subtle.digest('SHA-512',new Uint8Array(zip));
	const hashArray = Array.from(new Uint8Array(hashBuffer));
	// 将缓冲区转换为字节数组
	window.ZAPiX._lastDigest = hashArray.map(b => b.toString(16).padStart(2, '0')).join(''); 
	if (!isElectron())
		await window.ZAPiX.showDigest();
	
	window.ZAPiX._statusTextnode.data = "导出完成，SHA-512 哈希值已生成。";
	return window.ZAPiX._lastDigest;
}

window.ZAPiX._clearUI = function (){
	var uiElements = document.getElementById('zapixweb_header_div');
	while (uiElements!=null){
		uiElements.remove();
		uiElements = document.getElementById('zapixweb_header_div');
	}
}

window.ZAPiX._bootstrap = function(){
	// 初始化
	console.clear();
	window.ZAPiX._clearUI();
	console.log(window.ZAPiX._zapix_header);
	console.log('请稍候。提取完成后请保存文件。');
	var zapixweb_html = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|_|_|&nbsp;&nbsp;&nbsp;_|_|_|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|_|_|	<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|&nbsp;&nbsp;_|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|	<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|_|&nbsp;&nbsp;&nbsp;&nbsp;_|_|_|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|	<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|&nbsp;&nbsp;&nbsp;_|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|	<br>&nbsp;&nbsp;&nbsp;&nbsp;_|_|_|&nbsp;&nbsp;&nbsp;_|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_|_|_|	<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ZAPiX Web<br>WHATSAPP WEB&nbsp;&nbsp;&nbsp;&nbsp;数据提取器<br><br>';
	zapixweb_html += '<div id="zapixweb_status">正在启动...</div>';
	alert('     _|_|_|   _|_|_|     |_|_|	\n    _|        _|  _|       _|	\n      _|_|    _|_|_|       _|	\n         _|   _|           _|	\n    _|_|_|    _|         _|_|_|	\n	                      ZAPiX Web\nW H A T S A P P W E B  数 据 提 取 器\n 请按照控制台窗口中的提示操作。');
	
	window.ZAPiX._injectedHeaderNode = document.createElement("DIV");
	window.ZAPiX._injectedHeaderNode.id = 'zapixweb_header_div'
	window.ZAPiX._injectedHeaderNode.setAttribute('align','middle');
	window.ZAPiX._injectedHeaderNode.innerHTML = '<img src="data:image/png;base64,'+window.ZAPiX._logo+'" alt="ZAPiXWEB 标志" />'
	//var headerTextnode = document.createTextNode('Z A P i X W E B');
	//headerTextnode.id = 'zapixweb_header'
	//window.ZAPiX._injectedHeaderNode.appendChild(headerTextnode); 
	document.getElementById("pane-side").previousSibling.appendChild(window.ZAPiX._injectedHeaderNode);
	

	window.ZAPiX._mainMenu = document.createElement("DIV");
	window.ZAPiX._mainMenu.setAttribute("class","app-wrapper-web");
	window.ZAPiX._mainMenu.setAttribute('align','middle');
	window.ZAPiX._mainMenu.id = 'zapixweb_menu_div'
	
	//var style = document.createElement('style');
	//style.innerHTML = '{border: none;padding: 7px 26px;text-align: center;color: white;background: #dc3545;border-radius: 40px;}';
	var s = 'width: 100%;border: 1px solid black;padding: 1px 26px;text-align: left;color: white;background: forestgreen;border-radius: 40px;';
	var s2 = 'width: 100%;border: 1px solid black;padding: 1px 26px;text-align: left;color: white;background: darkgreen;border-radius: 40px;';
	//document.head.appendChild(style);
	window.ZAPiX._mainMenu.innerHTML = "菜单</br> <button id='zapixGetCurrentChatButton' title='将当前选中的聊天加入导出内容。' style='"+s+"'>添加当前聊天</button></br><button title='将所有聊天加入导出内容。' id='zapixGetAllChatsButton' style='"+s+"'>提取全部聊天</button></br><button id='zapixTakeoutButton' title='将所有数据打包为 ZIP 文件。' style='"+s+"'>打包导出</button></br><button id='zapixShowDigestButton' title='显示上次计算的哈希值。' style='"+s+"'>上次哈希</button></br><button id='zapixCloseButton' title='退出 ZAPiXWEB。' style='"+s2+"'>关闭</button></br>"
	//window.ZAPiX.getchat(this)
    window.ZAPiX._mainMenu.getElementsByTagName("button").namedItem("zapixGetCurrentChatButton").addEventListener("click", function(){window.ZAPiX.getchat(this)});
    window.ZAPiX._mainMenu.getElementsByTagName("button").namedItem("zapixGetAllChatsButton").addEventListener("click", function(){window.ZAPiX.getall(this)});
    window.ZAPiX._mainMenu.getElementsByTagName("button").namedItem("zapixTakeoutButton").addEventListener("click", function(){window.ZAPiX.takeout(this)});
    window.ZAPiX._mainMenu.getElementsByTagName("button").namedItem("zapixShowDigestButton").addEventListener("click", function(){window.ZAPiX.showDigest(this)});
    window.ZAPiX._mainMenu.getElementsByTagName("button").namedItem("zapixCloseButton").addEventListener("click", function(){window.ZAPiX.closeUI(this)});
    
    window.ZAPiX._injectedHeaderNode.appendChild(window.ZAPiX._mainMenu);
	
	window.ZAPiX._injectedStatusNode = document.createElement("DIV");
	window.ZAPiX._injectedStatusNode.id = 'zapixweb_status_div'
	window.ZAPiX._injectedStatusNode.setAttribute('align','middle');
	window.ZAPiX._injectedStatusNode.setAttribute('style','font-size:12px; border: 1px solid green');

	window.ZAPiX._statusTextnode = document.createTextNode('正在启动...');
	window.ZAPiX._statusTextnode.id = 'zapixweb_status';
	window.ZAPiX._injectedStatusNode.appendChild(window.ZAPiX._statusTextnode); 
	//document.getElementById("pane-side").previousSibling.appendChild(window.ZAPiX._injectedStatusNode);
	window.ZAPiX._injectedHeaderNode.appendChild(window.ZAPiX._injectedStatusNode);
	
	window.ZAPiX._internal_start()
	
	console.log(window.ZAPiX._zapix_header);
	//console.log('要提取所有聊天，请单击“提取全部聊天”');
	//console.log('要提取指定聊天，请先打开该聊天，然后单击“添加当前聊天”');
	//console.log('可以选择其他聊天并重复上述操作，以提取多个聊天');

	window.ZAPiX._statusTextnode.data = "请在 ZAPiX 菜单中选择要执行的操作";
}

window.ZAPiX.getall = async function (btn){
	if (window.ZAPiX._isExtractingAllChats === true) {
		console.warn('ZAPiX：全部聊天提取任务正在运行，请勿重复点击。');
		return null;
	}
	var takeoutButton = document.getElementById('zapixTakeoutButton');
	var takeoutWasDisabled = takeoutButton ? takeoutButton.disabled : false;
	window.ZAPiX._isExtractingAllChats = true;
	btn.style.background = 'darkgreen';
	btn.disabled = true;
	if (takeoutButton) takeoutButton.disabled = true;
	console.log(window.ZAPiX._zapix_header);
	console.log('ZAPiX：开始提取全部聊天。看到最终附件汇总前，请勿执行“打包导出”。');
	try{
		var summary = await window.ZAPiX._internal_getallchats();
		var details = '附件共 '+summary.totalAttachments+
			' 个：原文件下载 '+summary.downloadedMedia+
			' 个，预览替代 '+summary.previewSaved+
			' 个，vCard 保存 '+summary.vcardSaved+
			' 个，失败 '+summary.failed+' 个。';
		console.log('ZAPiX：附件处理已经结束。'+details);
		if (summary.totalAttachments === 0) {
			window.ZAPiX._statusTextnode.data = "全部聊天已提取，未发现附件，可以打包导出。";
			console.log('ZAPiX：全部聊天提取完成，未发现附件。现在可以点击“打包导出”。');
		} else if (summary.failed === 0 && summary.previewSaved === 0) {
			window.ZAPiX._statusTextnode.data = "全部聊天和附件已提取，可以打包导出。";
			console.log('ZAPiX：全部附件下载完成——所有媒体原文件均下载成功。现在可以点击“打包导出”。');
		} else if (summary.failed === 0) {
			window.ZAPiX._statusTextnode.data = "附件处理完成，但有原文件使用预览替代；请查看控制台。";
			console.warn('ZAPiX：全部附件处理结束，但有 '+summary.previewSaved+' 个原文件下载失败，已用内嵌预览替代。请核对后再打包导出。');
		} else {
			window.ZAPiX._statusTextnode.data = "附件处理结束，但有 "+summary.failed+" 个失败；请查看控制台。";
			console.error('ZAPiX：附件处理结束，但有 '+summary.failed+' 个附件未能保存。打包导出前请检查 FAILED_*.json 记录。');
		}
		return summary;
	}catch(error){
		var message = '全部聊天提取意外中止：'+(error && error.message ? error.message : error);
		window.ZAPiX._statusTextnode.data = message;
		console.error('ZAPiX：'+message, error);
		return null;
	}finally{
		window.ZAPiX._isExtractingAllChats = false;
		btn.disabled = false;
		btn.style.background = 'forestgreen';
		if (takeoutButton) takeoutButton.disabled = takeoutWasDisabled;
	}
}

window.ZAPiX.getchat = function (btn){
	if (document.getElementById("main") == null){
		alert('     _|_|_|   _|_|_|     |_|_|	\n    _|        _|  _|       _|	\n      _|_|    _|_|_|       _|	\n         _|   _|           _|	\n    _|_|_|    _|         _|_|_|	\n	                      ZAPiX Web\nW H A T S A P P W E B  数 据 提 取 器\n 执行命令前，请先用鼠标单击并打开要提取的聊天。');
		
		console.log('执行命令前，请先用鼠标单击并打开要提取的聊天。');
		return;
	}
	btn.style.background = 'darkgreen';
	spanIndex = 0;
	var chatTitle = document.getElementById("main").getElementsByTagName('span')[spanIndex];
	while (chatTitle.dir != 'auto'){
		chatTitle = document.getElementById("main").getElementsByTagName('span')[++spanIndex];
	}
	console.log(document.getElementById("main").getElementsByTagName('span')[spanIndex].title)
	chatName=document.getElementById("main").getElementsByTagName('span')[spanIndex].title;
	
	if (chatName=='' ){
		chatName=document.getElementById("main").getElementsByTagName('span')[spanIndex].innerHTML
	}
	//window.ZAPiX._internal_getchat(document.getElementById("main").getElementsByTagName('span')[spanIndex].title)
	window.ZAPiX._internal_getchat(chatName)
	btn.style.background = 'forestgreen';

	console.log(window.ZAPiX._zapix_header);
	console.log('要提取指定聊天，请打开该聊天，然后单击“添加当前聊天”。');
	console.log('可以选择其他聊天并重复上述操作，以提取多个聊天。');
	console.log('完成后，请单击“打包导出”。');
}

window.ZAPiX.takeout = async function (btn){
	console.clear();
	console.log(window.ZAPiX._zapix_header);
	console.log('正在生成导出文件。请等待保存对话框，这可能需要几分钟。');
	window.ZAPiX._statusTextnode.data = "正在打包导出数据...（请等待保存对话框）";
	console.log('文件保存后，请继续等待哈希值计算完成。');
	btn.style.background = 'darkgreen';
	btn.disabled = true;
	try{
		await window.ZAPiX._internal_end();
		console.log('ZIP 文件和 SHA-512 哈希值已生成。');
		console.log('可以单击“上次哈希”再次下载哈希文件。');
		console.log('祝取证工作顺利！');
	}catch(error){
		var message = "打包导出失败："+(error && error.message ? error.message : error);
		window.ZAPiX._statusTextnode.data = message;
		console.error(message, error);
		alert(message);
	}finally{
		btn.disabled = false;
		btn.style.background = 'forestgreen';
	}
}
window.ZAPiX.closeUI = function (btn){
	if (confirm('Z A P i X W E B \n 确定要退出吗？')){
		console.clear();
		btn.style.background = 'darkgreen';
		window.ZAPiX._clearUI();
		btn.style.background = 'forestgreen';
		console.log('请关闭控制台窗口（大多数浏览器可按 F12）。');
		console.clear();
	}
}

window.ZAPiX._bootstrap();

// 现在按回车键执行此脚本！
